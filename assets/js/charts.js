...
function fetchDataAndUpdateCharts() {
  fetch('execute_python.php')
    .then(response => response.json())
    .then(data => {
      if (data.status === 'error') {
        console.error('Python Script Error:', data.message);
      } else {
        console.log('Python Script Success:', data.message);

        // Fetch updated data for chart 1
        fetch('fetch_dbChartOne.php')
          .then(response => response.json())
          .then(data1 => {
            console.log(data1);
        ...
        // Fetch updated data for chart 2
        fetch('fetch_dbChartTwo.php')
          .then(response => response.json())
          .then(data2 => {
            console.log(data2);
...
// Set interval to fetch and update every 5 minutes (300000 milliseconds)
